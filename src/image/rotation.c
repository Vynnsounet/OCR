...
{
    Uint8 *const target_pixel = ((Uint8 *)surface->pixels + y * surface->pitch + x * surface->format->BytesPerPixel);
    return target_pixel;
}
...
void put_pix(SDL_Surface *surface, unsigned x, unsigned y, Uint32 pixel)
{
...
SDL_Surface *size(float a, int h, int w)
{
    ...
    else
    {
        ...
        SDL_Surface *new = SDL_CreateRGBSurface(0, sqrt((wx - wy) * (wx - wy) + (hx - hy) * (hx - hy)),
                                                sqrt((wx - wy) * (wx - wy) + (hx - hy) * (hx - hy)), 32, 0, 0, 0, 0);
        return new;
    }
}
